    ...
    def update(self, centroids):
        ...
        if len(centroids) == 0:
            for obj_id in list(self.objects.keys()):
                ...
                if self.missing[obj_id] > self.max_missing:   # hilang kelamaan
        ...
        for c in centroids:
            ...
            if best_id is not None:              # orang lama
                ...
                self._record(best_id, c)         # ← catat
            else:                                # orang baru
                new_objects[self.next_id] = c
                new_missing[self.next_id] = 0
                self._record(self.next_id, c)    # ← catat
                self.next_id += 1
                
        # pertahankan objek lama yang nggak kecocok frame ini (masih dalam toleransi)
        for obj_id, pos in self.objects.items():
            if obj_id not in used_ids:
                miss = self.missing.get(obj_id, 0) + 1
                if miss <= self.max_missing:
                    new_objects[obj_id] = pos    # simpan posisi terakhir
                    new_missing[obj_id] = miss

        self.objects = new_objects
        self.missing = new_missing
        return self.objects
    def _record(self, obj_id, centroid):
        # simpan posisi ke riwayat, buang yang paling lama kalau kepanjangan
        if obj_id not in self.history:
            self.history[obj_id] = []
        self.history[obj_id].append(centroid)
        if len(self.history[obj_id]) > self.history_length:
            self.history[obj_id].pop(0)      # buang yang paling lama